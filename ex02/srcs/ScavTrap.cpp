...
/*   Updated: 2023/04/24 20:58:55 by plau             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	std::cout << BLUE << "[ScavTrap] Copy Constructor for " << CYAN << &src << RESET << '\n';
	(*this) = src;
}
...
ScavTrap& ScavTrap::operator=(const ScavTrap& src)
{
	if (this != &src) //check for self-assignment
	{
		...
		std::cout << MAGENTA << "[ScavTrap] Assignment Operator " << this->_name 
...
void	ScavTrap::attack(const std::string& target)
{
	...
	{
		std::cout << RED << "[ScavTrap] [" << this->_name << " can't attack] No hit points left" << RESET << '\n';
		return ;
	}
	if (this->_energyPoints < 1)
	{
		std::cout << RED << "[ScavTrap] [Can't attack] No energy points left" << RESET << '\n';
		return ;
	}
	this->_energyPoints--;
